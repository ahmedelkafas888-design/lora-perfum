...
// Initial Core Vault Data Mapping
...
// CRITICAL FIX: Safe initialization sequence that never wipes existing inventory modifications
function initApp() {
...
function updateCartItemQty(productId, delta) {
    ...
    if (idx > -1) {
        ...
        if (cart[idx].quantity <= 0) {
            cart.splice(idx, 1);
            const isAr = getCurrentLang() === 'ar';
            showToastNotification(isAr ? "✓ تم حذف المنتج من العربة" : "✓ Product removed");
        }
        localStorage.setItem('lora_cart', JSON.stringify(cart));
        updateCartBadge();
    }
